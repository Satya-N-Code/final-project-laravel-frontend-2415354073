...
<div class="flex flex-col gap-5" x-data="{ 
            ...
            <tbody class="text-gray-700 text-sm">
                ...
                @forelse ($services as $service)
                    <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors relative">
                        ...
                        <td class="py-4 px-6">
                            @if(in_array(strtolower((string)($service['status'] ?? '0')), ['active', '1', 'true']))
                        ...
                        <td class="py-4 px-6 text-center" x-data="{ open: false }">
                            <div class="relative inline-block text-left">
                                ...
                                <div x-show="open" style="display: none;" 
                                    ...
                                    @if(!in_array(strtolower((string)($service['status'] ?? '0')), ['active', '1', 'true']))
                                    <form action="{{ route('services.updateStatus', $service['id']) }}" method="POST" class="w-full m-0 p-0">
                                        @csrf
                                        @method('PATCH')
                                    ...
                                    <button @click="
                                        editForm = {
                                            ...
                                            name: '{{ $service['name'] }}',
                                            price: '{{ $service['price'] ?? '' }}',
                                            status: '{{ (in_array(strtolower((string)($service['status'] ?? '0')), ['active', '1', 'true'])) ? '1' : '0' }}'
                                        };
                                        showEditModal = true;
                                        open = false;
                ...
                @empty
                    <tr>
                        <td colspan="4" class="py-12 text-center text-gray-500 font-medium">No service data available.</td>
                    </tr>
                @endforelse
    ...
    <div x-show="showAddModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px]" style="display: none;">
        <div @click.outside="showAddModal = false" class="bg-white w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-100"><h3 class="text-xl font-bold text-gray-800 text-center">Add Service</h3></div>
            <form action="{{ route('services.store') }}" method="POST" class="p-8 space-y-5">
                @csrf
                ...
                <div class="space-y-2">
                    <label class="block text-sm font-bold text-gray-700">Service Name</label>
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="Enter service name" class="w-full px-4 py-3 rounded-xl border {{ $errors->has('name') && !old('_method') ? 'border-red-500' : 'border-gray-200' }} text-sm">
                </div>
                
                <div class="space-y-2">
                    <label class="block text-sm font-bold text-gray-700">Price (Rp)</label>
                    <input type="number" name="price" value="{{ old('price') }}" placeholder="Enter price, e.g. 75000" class="w-full px-4 py-3 rounded-xl border {{ $errors->has('price') && !old('_method') ? 'border-red-500' : 'border-gray-200' }} text-sm">
                </div>
    ...
    <div x-show="showEditModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px]" style="display: none;">
        <div @click.outside="showEditModal = false" class="bg-white w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-100"><h3 class="text-xl font-bold text-gray-800 text-center">Edit Service</h3></div>
            
            <form x-bind:action="'/services/' + editForm.id" method="POST" class="p-8 space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    <label class="block text-sm font-bold text-gray-700">Service Name</label>
                    <input type="text" name="name" x-model="editForm.name" placeholder="Enter service name" class="w-full px-4 py-3 rounded-xl border {{ $errors->has('name') && old('_method') == 'PUT' ? 'border-red-500' : 'border-gray-200' }} text-sm">
                </div>
                
                <div class="space-y-2">
                    <label class="block text-sm font-bold text-gray-700">Price (Rp)</label>
                    <input type="number" name="price" x-model="editForm.price" placeholder="Enter price, e.g. 75000" class="w-full px-4 py-3 rounded-xl border {{ $errors->has('price') && old('_method') == 'PUT' ? 'border-red-500' : 'border-gray-200' }} text-sm">
                </div>
    ...
    <div x-show="showDeleteModal" 
        ...
        <div @click.outside="showDeleteModal = false" 
            ...
            <p class="text-sm text-gray-500 mb-6">Are you sure you want to delete this service? This action cannot be undone.</p>
